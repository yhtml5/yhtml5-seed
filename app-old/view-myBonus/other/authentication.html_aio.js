...
reqwest({
    ...
    method: 'get',
    type: 'json',
    data: {new_url: zjyData.wechat.url},
    ...
    success: function (resp) {
        console.log('getWechatConfig: ', resp)
        zjyData.wechat.appId = resp.appId
        zjyData.wechat.nonceStr = resp.nonceStr
        zjyData.wechat.signature = resp.signature
        zjyData.wechat.timestamp = resp.timestamp
        wx.config({
            debug: false, // 开启调试模式,调用的所有api的返回值会在客户端alert出来，若要查看传入的参数，可以在pc端打开，参数信息会通过log打出，仅在pc端时才会打印。
            appId: zjyData.wechat.appId, // 必填，公众号的唯一标识
            timestamp: zjyData.wechat.timestamp, // 必填，生成签名的时间戳
            nonceStr: zjyData.wechat.nonceStr, // 必填，生成签名的随机串
            signature: zjyData.wechat.signature,// 必填，签名，见附录1
            jsApiList: [
                'checkJsApi',
                'chooseImage',
                'previewImage',
