...
class communityManagerEdit extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            isUpload:true,
        ...
        this.props.dispatch(actions.communityShow({
           community_id: ID
        }));
    ...
    handleSubmit(){
        ...
        this.props.form.validateFields((err, values) => {
            ...
            // console.log(values.logo_url[0].response.data.imgPath+values.logo_url[0].response.data.message)
            if(values.logo_url[0].response!=undefined){
              logo_urls=values.logo_url[0].response.data.imgPath+values.logo_url[0].response.data.message
            }else{
                logo_urls=values.logo_url[0].url
            }
            for(var i=0;i<values.img_list.length;i++){
                if(values.img_list[i].response!=undefined){
                  img_lists[i]=values.img_list[i].response.data.imgPath+values.img_list[i].response.data.message
                }else{
                    img_lists[i]=values.img_list[i].url
                }
            }
             var ID = this.props.location.query.id;
  ...
  for (var i=0;i<arguments[2].allCheckedNodes.length;i++){
    ...
    render() {
        ...
        const {
            ...
            info,
            uploadToken
            } = this.props.communityManagerEdit;
        ...
        return (
            ...
            <Breadcrumb.Item>小区编辑</Breadcrumb.Item>
            </Breadcrumb>
            <Card className="mb1">
 <Form>
        <FormItem label="小区编号" {...Layout}>
            {getFieldDecorator('community_no', {
                rules: [
                    {required: true,pattern: /^[0-9a-zA-Z]{1,20}$/, message: '请输入小区编号!',type: "string"},
                ],
                initialValue:info.community_no
            })(
        ...
        <Cascader options={options}  placeholder="小区区域"  />
            ...
            {getFieldDecorator('group', {
                ...
                <Input placeholder="苑/期/区"  maxLength={15}/>
            )}
        </FormItem>
            ...
            {getFieldDecorator('pro_company_id', {
                ...
                initialValue:info.pro_company_id
            })(
            ...
            {getFieldDecorator('service_list', {
                ...
                initialValue:info.service_list
            })(
            <TreeSelect {...tProps} />
            )}
        </FormItem>
            ...
            {this.state.isUpload&&info.logo_url&&info.logo_url.length!=1?uploadButton:null}
        </Upload>
        )}
         <Modal visible={this.state.previewVisible} footer={null} onCancel={this.handleCancel.bind(this)}>
          <img alt="example" style={{ width: '100%' }} src={this.state.previewImage} />
            ...
            {this.state.isUpload2&&info.img_list&&info.img_list.length!=3?uploadButton:null}
        </Upload>
        )}
        <Modal visible={this.state.previewVisible} footer={null} onCancel={this.handleCancel.bind(this)}>
          <img alt="example" style={{ width: '100%' }} src={this.state.previewImage} />
...
communityManagerEdit = createForm()(communityManagerEdit);
export default connect(state => {
    return {
        communityManagerEdit: state.communityManagerEdit
    }
