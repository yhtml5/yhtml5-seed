...
class Component extends React.Component {
  constructor(props) {
    ...
    // props.form.getFieldDecorator('keys', { initialValue: [1] })
  ...
  setForm(props) {
    if (props.value) {
      ...
      props.form.getFieldDecorator(`formColumnTypeId`, { initialValue: props.value.type_id })
      // props.form.getFieldDecorator('items', { initialValue: props.value.items })
  ...
  render() {
    ...
    const formItems = keys.map((k, index) => {
      return (
        <div key={index}>
          <Form.Item label="链接图片" {...formItemLayout} key={k} required={false}>
          ...
          <Form.Item label="菜单名称" labelCol={{ span: 8 }} wrapperCol={{ span: 12 }}>
            {this.props.form.getFieldDecorator(`formMenuName${k}`, {
