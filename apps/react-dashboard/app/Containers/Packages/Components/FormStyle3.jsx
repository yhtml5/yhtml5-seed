...
class Component extends React.Component {
  constructor(props) {
    ...
    process.env.NODE_ENV === 'production' || console.warn('FormStyles3', props)
  ...
  setForm(props) {
    if (props.value) {
      ...
      props.value.items.forEach((value, index) => {
        ...
        props.form.getFieldDecorator(`formIntro${uuid}`, { initialValue: value.image_intro })
  ...
  componentDidMount() {
    console.log('formStyle1DidMount', this.props)
    ...
    this.props.form.validateFields((err, values) => {
      ...
      const getItems = () => keys.map((k) => {
        return {
          ...
          image_intro: this.props.form.getFieldValue(`formIntro${k}`),
        }
      })
        ...
        <div key={index}>
          ...
          <Form.Item label="url" {...formItemLayout}>
            {this.props.form.getFieldDecorator(`formUrl${k}`, {
              ...
              <Input type="textarea" placeholder="请输入url" />
              )}
          </Form.Item>
          <Form.Item label="图片描述" labelCol={{ span: 8 }} wrapperCol={{ span: 12 }}>
            {this.props.form.getFieldDecorator(`formIntro${k}`, {
              // initialValue: `${this.props.step}ImageIntroduction${k}`,
              rules: [
                { required: true, message: '请输入图片描述!' },
                { type: "string", pattern: /^.{0,36}$/, message: '最大输入36个字!' }
              ],
            })(
              <Input type="text" placeholder="请输入图片描述" style={{ width: '80%', marginRight: 8 }} />
              )}
            <Icon
